...
async def get_threads(request: Request, x_guest_id: str = Depends(require_guest_id)):
    threads = await request.app.state.service.get_threads(x_guest_id)
...
async def create_thread(request: Request, request_body: PostThreadRequest, x_guest_id: str = Depends(require_guest_id)):
    thread_id = await request.app.state.service.create_thread(x_guest_id, request_body.query)
...
async def update_thread(request: Request, thread_id: str, x_guest_id: str = Depends(require_guest_id)):
    response = await request.app.state.service.update_thread(thread_id, x_guest_id)
    if not response:
...
async def delete_thread(request: Request, thread_id: str, x_guest_id: str = Depends(require_guest_id)):
    response = await request.app.state.service.delete_thread(thread_id, x_guest_id)
    if not response:
        raise HTTPException(status_code=404, detail="Thread not found")
    return {"status": "ok"}
...
async def get_messages(request: Request, thread_id: str, x_guest_id: str = Depends(require_guest_id)):
    messages = await request.app.state.service.get_messages(thread_id, x_guest_id)
    if messages is None:
        raise HTTPException(status_code=404, detail="Thread not found")
    return GetMessagesResponse(messages=messages)

@api_router.post("/threads/{thread_id}/messages")
async def stream(request: Request, request_body: PostMessagesRequest, thread_id: str, x_guest_id: str = Depends(require_guest_id)):
    async_stream = await request.app.state.service.stream(thread_id, x_guest_id, request_body.query)
